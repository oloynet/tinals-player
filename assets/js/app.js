...
async function init() {
    try {
        ...
        if( favsParam ) {
            ...
            setVideoId( Number( idParam ) );
            updateProgramPlayingState( getVideoId() );
...
function handleOrientationChange() {
    ...
    if( getVideoId() !== null ) {
        VideoManager.applyMobileZoom( getVideoId() );
    }
    ...
    const player = VideoManager.instances[ getVideoId() ];
...
function toggleAtAGlanceDrawer() {
    ...
    if( drawer.classList.contains( 'active' ) ) {
        ...
        if( getVideoId() ) {
             ...
             setTimeout( () => {
                 scrollToAtAGlanceItem( getVideoId() );
             }, 300 );
        }
    }
}
...
function toggleFavTimelineDrawer( tabName ) {
    ...
    if( drawer.classList.contains( 'active' ) && getActiveTab() === 'favorites' ) {
...
function updateURLState() {
    ...
    if( isFilterMode( 'favorite' ) && AppState.favorites.length > 0 ) {
        ...
        url.searchParams.set( 'id', getVideoId() );
    }
    else if( getActiveSession() ) {
        url.hash = getActiveSession();
    }
...
async function shareCurrent() {
    ...
    if( isFilterMode( 'favorite' ) && AppState.favorites.length > 0 ) {
        ...
        urlObj.searchParams.set( 'id', getVideoId() );
    }
        ...
        entries.forEach( entry => {
            ...
            if( entry.isIntersecting ) {
                ...
                if( entry.target.hasAttribute( 'data-id' ) ) {
                    ...
                    else if( getPreviousVideoId() !== null && VideoManager.instances[ getPreviousVideoId() ] ) {

                        if( typeof VideoManager.instances[ getPreviousVideoId() ].getPlayerState === 'function' ) {

                            const prevState = VideoManager.instances[ getPreviousVideoId() ].getPlayerState();
...
function setupKeyboardControls() {
    document.addEventListener( 'keydown', ( e ) => {
        ...
        if( e.key === 'Escape' ) {
            ...
            if( drawers.length > 0 || getIsMainMenu() ) {
                closeFavTimelineDrawers();
                return;
            }
...
function setFilterTag( filter ) {
    AppState.state.currentTagFilter = filter;
}
...
function updateFilterBarText() {
    ...
    if( isFilterMode( 'favorite' ) ) {
        ...
        if( getVideoId() && AppState.favorites.includes( getVideoId() ) ) { // Find position of current activeId in favorites list ( data order)
        ...
        if( getVideoId() ) {
            const index = matchingItems.findIndex( g => g.id === getVideoId() );
...
function closeDrawerIfOpen() {
    ...
    if( drawer.classList.contains( 'active' ) || getIsMainMenu() || ( atAGlance && atAGlance.classList.contains( 'active' ) ) ) {
        closeFavTimelineDrawers();
        return true;
    }
